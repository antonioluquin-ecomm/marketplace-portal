...
- **Auth por sesión (desde 2026-07-02)**: `internal/` requiere login de staff (`login.html`, `assets/js/auth.js`, RBAC por módulo) y `public/` requiere login de Seller (`public/login.html`, `assets/js/auth-seller.js`). Backend en `apps-script/Users.gs`. **Desde 2026-07-16, un mismo `seller_id` puede tener varias cuentas Seller** (una por persona del equipo del seller, cada una con su propio nombre/email/contraseña) — ya no hay restricción de unicidad; el scope de datos se resuelve por el `seller_id` de la fila de USUARIOS de cada sesión, no por una cuenta única. Ver `CHANGELOG.md` para el detalle de cada etapa.
