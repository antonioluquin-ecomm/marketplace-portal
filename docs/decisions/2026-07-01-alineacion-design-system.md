...
8. **Etapa 6 (capa JS compartida de `fetch`) evaluada y descartada:** el plan original la marcaba como opcional, sujeta a análisis costo/beneficio. Relevamiento real: ~4 escrituras (`fetch` con `mode:'no-cors'`) y ~9 lecturas CSV repartidas en `backlog-sellers.html`, `gestion-sellers.html` y `config-tarifas.html` — las tres herramientas de escritura de uso diario del equipo (pipeline, alta de sellers, tarifas/overrides con `write_secret`). Como `no-cors` deja la respuesta opaca, un wrapper compartido no mejora el manejo de errores más allá de lo que ya existe; el único beneficio real es reducir ~5-10 líneas de duplicación por call site. El riesgo es desproporcionado: un bug en el wrapper rompería los tres flujos de escritura a la vez, en vez de quedar aislado a un archivo. Decisión explícita del usuario: no ejecutar.
...
- **Punto 7 (public/ excluido, tema oscuro) queda revertido**: las 7 páginas de `public/` migran a modo claro + `tokens.css`/`internal-components.css`, igual que `internal/`, en vez de mantener su paleta oscura propia (`#09100a` de fondo, verde `#5ea832`/`#6aa84f`). Ejecutado en 4 etapas con checkpoint (ver `CHANGELOG.md`, 2026-07-06): tokens + limpieza de `internal/`, luego `public/` página por página (index/login/gantt-seller, formularios, presentación/simulador) por ser el código de mayor riesgo (lo usa el seller externo en producción vía `?seller_id=`/sesión de seller). Completado.
